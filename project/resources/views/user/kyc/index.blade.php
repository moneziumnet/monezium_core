                        ...
                        <form action="{{route('user.kyc.submit')}}" method="POST" enctype="multipart/form-data">
                            ...
                            @foreach (json_decode($userForms->data) as $field)
                                ...
                                <div class="form-group mb-3 mt-3">
                                    ...
                                    @if ($field->type == 1)
                                    <input pattern="[^À-ž()/><\][\\;&$@!|]+" type="text" name="{{strtolower(str_replace(' ', '_', $field->label))}}" class="form-control" autocomplete="off" placeholder="@lang($field->label)" min="1" {{$field->required == 1 ? 'required':'Optional'}}>
                                    @else
