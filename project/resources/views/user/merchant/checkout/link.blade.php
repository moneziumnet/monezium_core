<html lang="en">
  <head>
    ...
    <title>{{__($gs->title)}} - @lang('Checkout : '.$checkout->name) </title>
    <!-- CSS files -->
                          ...
                          <div class="col-md-5 ">
                            <form action="{{route('user.merchant.checkout.transaction')}}" method="post" id="form_submit" class="text-end">
                                ...
                                <div class="form-group ms-5 mt-5 text-start" >
                                    ...
                                    @if ($checkout->amount == 0 || $checkout->amount == null)
                                    <input name="amount" id="amount" class="form-control shadow-none col-md-4" autocomplete="off"  type="number" value="{{ $checkout->amount }}" required>
                                    @else
                                    <input name="amount" id="amount" class="form-control shadow-none col-md-4" autocomplete="off"  type="number" value="{{ $checkout->amount }}" readonly required>
                                    @endif
                                </div >
                                ...
                                <div class="form-group ms-5 mt-3 text-start user-name" >
                                    ...
                                    <input name="user_name" id="user_name" class="form-control shadow-none col-md-4" type="text" pattern="[^À-ž()/><\][\\;&$@!|]+" required>
                                </div >
                                @endif
      ...
      <script>
        ...
        $('#btn-pay').on('click', function(e) {
            ...
            if(payment_type == "bank_pay" && document.getElementById('form_submit').checkValidity()) {
                ...
                $('#detail_amount').html("{{$checkout->currency->symbol}}" + $('#amount').val());
