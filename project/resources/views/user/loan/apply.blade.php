            ...
            <div class="col-md-12">
                <div class="card p-4">
                    <table class="table table-transparent table-responsive">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-end">{{ $loanAmount }} {{$currencyinfo->code}}</td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td class="text-end">{{ ($loanAmount * $data->per_installment/100)}} {{$currencyinfo->code }}</td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td class="text-end text-danger">{{ $perInstallment * $data->total_installment }} {{$currencyinfo->code}}</td>
                            </tr>
                    ...
                    <form action="{{route('user.loan.request')}}" method="POST" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="per_installment_amount" value="{{ ($loanAmount * $data->per_installment)/100 }}">
                        ...
                        @if ($data->required_information)
                            @foreach (json_decode($data->required_information,true) as $key=>$value)
                                ...
                                @if ($value['type'] == 'text')
                                    <div class="form-group mb-3 mt-3">
                                        <label class="form-label" {{$value['validation']}}> {{$value['field_name']}} </label>
                                        <input type="text" pattern="[^À-ž()/><\][\\;&$@!|]+" name="{{$value['field_name']}}" placeholder="{{$value['field_name']}}" class="form-control" autocomplete="off" {{$value['validation']}}>
                                    </div>
                                @endif
