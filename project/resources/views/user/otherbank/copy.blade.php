                    ...
                    <form action="{{route('user.other.send.store')}}" method="POST" id="otherbank_form" enctype="multipart/form-data">
                        ...
                        <div class="form-group mt-3">
                            ...
                            <select name="subbank" id="withmethod" class="form-select" required>
                                ...
                                @foreach ($banks as $key => $bank)
                                        <option value="{{$bank->id}}" {{$beneficiary->subbank == $bank->id ? 'selected' : ''}} >{{$bank->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group mt-3">
                            ...
                            <select name="currency_id" id="currency" class="form-select" required>
                                ...
                                <option value="{{$beneficiary->currency_id}}" selected> {{$beneficiary->currency->code}} </option>
                            </select>
                        </div>
                        ...
                        <div class="form-group mt-3">
                            ...
                            <select name="payment_type" id="payment_type" class="form-select" required>
                                ...
                                <option value="SWIFT" {{$beneficiary->payment_type == 'SWIFT' ? 'selected' : ''}}>{{__('SWIFT')}}</option>
                                <option value="SEPA" {{$beneficiary->payment_type == 'SEPA' ? 'selected' : ''}}>{{__('SEPA')}}</option>
                                <option value="SEPA_INSTANT" {{$beneficiary->payment_type == 'SEPA_INSTANT' ? 'selected' : ''}}>{{__('SEPA_INSTANT')}}</option>
                                <option value="CHAPS" {{$beneficiary->payment_type == 'CHAPS' ? 'selected' : ''}}>{{__('CHAPS')}}</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group mt-3">
                            ...
                            <textarea name="des" id="des" class="form-control" autocomplete="off" placeholder="{{__('Please input description')}}" type="text" required>{{$beneficiary->description}}</textarea>
                        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var amount = '{{$beneficiary->amount}}'
        if (parseFloat(amount) >= parseFloat('{{$other_bank_limit}}')) {
            document.getElementById("document").style.display = "block";
            document.getElementById("document_label").style.display = "block";
        }
        else {
