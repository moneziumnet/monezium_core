...
@if($viewType == 'default')
    ...
    <div class="message-card" data-id="{{ $id }}">
        ...
        <div class="image-file chat-image" style="width: 250px; height: 150px;background-image: url('{{ App\Facades\ChatifyMessenger::getAttachmentUrl($attachment[0]) }}')">
        </div>
        @endif
    </div>
    @endif
    ...
    <div class="message-card mc-sender" title="{{ $fullTime }}" data-id="{{ $id }}">
        <div class="chatify-d-flex chatify-align-items-center" style="flex-direction: row-reverse; justify-content: flex-end;">
            ...
            <p style="margin-left: 5px;">
                ...
                {{-- If attachment is a file --}}
                @if(@$attachment[2] == 'file')
                <a href="{{ storage_path('app/public') . ['fileName'=>$attachment[0]] }}" class="file-download">
                    <span class="fas fa-file"></span> {{$attachment[1]}}</a>
                @endif
            </p>
        </div>
        {{-- If attachment is an image --}}
        @if(@$attachment[2] == 'image')
        <div class="image-file chat-image" style="margin-top:10px;width: 250px; height: 150px;background-image: url('{{ App\Facades\ChatifyMessenger::getAttachmentUrl($attachment[0]) }}')">
        </div>
        @endif
    </div>
@endif
