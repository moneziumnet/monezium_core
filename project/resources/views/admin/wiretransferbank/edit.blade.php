...
<div class="card">
  ...
  <ol class="breadcrumb py-0 m-0">
      ...
      <li class="breadcrumb-item"><a href="{{route('admin.wire.transfer.banks.edit',$data->id)}}">{{ __('Add Wire Transfer Bank') }}</a></li>
  </ol>
  </div>
</div>
...
<div class="col-md-10">
  <div class="card mb-4">
    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
      <h6 class="m-0 font-weight-bold text-primary">{{ __('Edit Wire Transfer Bank Form') }}</h6>
    </div>

    <div class="card-body">
      <div class="gocover" style="background: url({{asset('assets/images/'.$gs->admin_loader)}}) no-repeat scroll center center rgba(45, 45, 45, 0.5);"></div>
      <form class="geniusform" action="{{route('admin.wire.transfer.banks.update',$data->id)}}" method="POST" enctype="multipart/form-data">
          ...
          <div class="row">
              ...
              <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="text" pattern="[^À-ž()/><\][\\;&$@!|]+" class="form-control" id="swift" name="swift_code" placeholder="{{ __('Enter Swift Code') }}" value="{{ $data->swift_code }}">
                </div>
              </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                  ...
                  <input type="number" class="form-control" id="fixed" name="fixed_charge" placeholder="{{ __('Enter Fixed Charge') }}" min="1" step="0.01" value="{{ $data->fixed_charge }}" required>
              </div>
            </div>
            ...
            <div class="col-md-6">
              <div class="form-group">
                  ...
                  <input type="number" class="form-control" id="percentage" name="percentage_charge" placeholder="{{ __('Enter Percentage Charge') }}" min="0" step="0.01" value="{{ $data->percentage_charge }}" required>
              </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                  ...
                  <input type="number" class="form-control" id="routing" name="routing_number" placeholder="{{ __('Enter Routing Number') }}" min="0" step="0.01" value="{{ $data->routing_number }}" required>
              </div>
            </div>
