    ...
    <div class="card-body">
      ...
      <form class="geniusform" action="{{route('admin.ps.update')}}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <input type="text"  pattern="[^À-ž()/><\][\\;&$@!|]+" class="form-control" id="inp-title" name="about_title"  placeholder="{{ __('Enter About Title') }}" value="{{ $data->about_title }}" required>
          </div>
                ...
                @if ($data->about_attributes)
                    @foreach (json_decode($data->about_attributes,true) as $key=>$data)
                        <div class="lang-area mb-3">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <input type="text"  pattern="[^À-ž()/><\][\\;&$@!|]+" class="form-control" name="about_attributes[]" placeholder="{{ __('Enter Anout Attribute') }}" value="{{ $data }}" required>
                                </div>
                            </div>
                        </div>
                    @endforeach
...
$("#lang-btn").on('click', function(){
    ...
    $("#lang-section").append(''+
                                '<div class="lang-area mb-3">'+
                                  ...
                                  '<div class="row">'+
                                    '<div class="col-md-12">'+
                                    '<input type="text"  pattern="[^À-ž()/><\\][\\\\;&$@!|]+" class="form-control" name="about_attributes[]" placeholder="{{ __('Enter About Attribute') }}" value="" required>'+
                                    '</div>'+
                                  '</div>'+
                                '</div>'+
                            '');
