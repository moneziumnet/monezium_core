...
<div class="card">
  <div class="d-sm-flex align-items-center justify-content-between">
    <h5 class=" mb-0 text-gray-800 pl-3">{{ __('Edit Plan') }} <a class="btn btn-primary btn-rounded btn-sm" href="{{route('admin.fdr.plan.index')}}"><i class="fas fa-arrow-left"></i> {{ __('Back') }}</a></h5>
    <ol class="breadcrumb">
  ...
  <div class="col-md-10">
    <div class="card mb-4">
      ...
      <div class="card-body">
        ...
        <form class="geniusform" action="{{route('admin.fdr.plan.update',$data->id)}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6">
                  <div class="form-group">
                      ...
                      <select name="interval_type" class="form-control" id="interval_type">
                          <option value="fixed" {{ $data->interval_type == 'fixed' ? 'selected' : '' }}> {{__('Fixed')}} </option>
                          <option value="partial" {{ $data->interval_type == 'partial' ? 'selected' : '' }}> {{__('Partial')}} </option>
                      </select>
                  </div>
                </div>
                ...
                <div class="col-md-6">
                  <div class="form-group">
                      ...
                      <input type="number" class="form-control" id="matured_days" name="matured_days" placeholder="{{ __('Locked In Period') }}" min="1" value="{{$data->matured_days}}" required>
                    </div>
                </div>
            </div>

            <div class="row interval_time {{ $data->interval_type == 'fixed' ? 'd-none' : '' }}">
              <div class="col-md-12">
                <div class="form-group">
                  <label for="interest_interval">{{ __('Interval Time') }}</label>
                  <input type="number" class="form-control" id="interest_interval" name="interest_interval" placeholder="{{ __('Interval Time') }}" min="1" value="{{$data->interest_interval}}">
                </div>
              </div>
            </div>
