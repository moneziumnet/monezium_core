      ...
      <div class="card-body">
        ...
        <form class="geniusform" action="{{route('admin.withdraw.method.store')}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="inp-fixed" placeholder="{{  __('Withdraw Fixed Fee')  }}" name="fixed"  value="" required>
            </div>

            <div class="form-group">
              <label for="inp-percentage">{{  __('Withdraw Percentage Charge')  }} <small>({{__('Leave 0 if you don\'t want to add')}})</small></label>
              <input type="text" class="form-control" id="inp-percentage" placeholder="{{  __('Withdraw Percentage Charge')  }}" name="percentage" value="" required>
            </div>
