...
<div class="modal modal-blur fade" id="modal-success" tabindex="-1" role="dialog" aria-hidden="true">
  ...
  <div class="modal modal-blur fade" id="editModal" tabindex="-1" role="dialog" aria-hidden="true">
    ...
    <div class="modal-content">
        ...
        <i  class="fas fa-info-circle fa-3x text-primary mb-2"></i>
        <h3>@lang('Plan Details')</h3>
        <form method="post" action="{{route('admin.custom.charge.all.update', $data->id)}}" class="mt-3">
          @csrf
          <div class="row">
            <div class="col-md-4">
          ...
          <div style="max-height: 600px;overflow-y: scroll; overflow-x: hidden;">
            ...
            <div class="row border-bottom mt-2 p-2 align-items-center">
              ...
              <input  type="hidden" name="plan_id_{{$item->id}}" class="form-control" value="{{$item->plan_id}}">
              <input  type="hidden" name="slug_{{$item->id}}" class="form-control" value="{{$item->slug}}">
              @php
                $customplan =  DB::table('charges')->where('user_id',$data->id)->where('plan_id', $item->plan_id)->where('name', $item->name)->first();
                if($customplan) {
    ...
    var table = $('#geniustable').DataTable({
        ...
        $(function() {
                $(".btn-area").append('<div class="col-sm-12 col-md-4 pr-3 text-right">'+
                    '<a href="javascript:;" data-toggle="modal" data-target="#editModal" class="btn btn-primary" data-href="{{route('admin.staff.create')}}">'+
