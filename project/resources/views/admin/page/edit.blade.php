      ...
      <div class="card-body">
        ...
        <form class="geniusform" action="{{route('admin.page.update',$data->id)}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" pattern="[^À-ž()/><\][\\;&$@!|]+" class="form-control" id="title" name="title" value="{{$data->title}}"  placeholder="{{ __('Title') }}" value="" required>
            </div>

            <div class="form-group">
                <label for="inp-slug">{{ __('Slug') }}</label>
                <input type="text" pattern="[^À-ž()/><\][\\;&$@!|]+" class="form-control" id="inp-slug" value="{{$data->slug}}" name="slug"  placeholder="{{ __('Enter Slug') }}" value="" required>
            </div>
          ...
          <div class="showbox d-none">
            <div class="form-group">
              ...
              <input type="text" pattern="[^À-ž()/><\][\\;&$@!|]+" class="form-control" id="meta_tag" name="meta_tag" value="{{$data->meta_tag}}" placeholder="{{ __('Meta Tags') }}">
            </div>

            <div class="form-group">
              <label for="meta_description">{{ __('Meta Description') }}</label>
              <textarea class="form-control summernote"  id="meta_description" name="meta_description" rows="3"  placeholder="{{__('Meta Description')}}">{{$data->meta_description}}</textarea>
          </div>
          </div>
