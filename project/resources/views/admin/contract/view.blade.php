...
<div class="card">
    ...
    <h5 class=" mb-0 text-gray-800 pl-3">{{ __('View Contract') }} <a class="btn btn-primary btn-rounded btn-sm" href="{{route('admin.contract.management', $data->user_id)}}"><i class="fas fa-arrow-left"></i> {{ __('Back') }}</a></h5>
    <ol class="breadcrumb m-0 py-0">
        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>
        <li class="breadcrumb-item"><a href="{{ route('admin.contract.management', $data->user_id) }}">{{ __('Contracts') }}</a></li>
    </ol>
    </div>
</div>
