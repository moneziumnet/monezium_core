...
<div class="card">
  <div class="d-sm-flex align-items-center justify-content-between py-3">
    <h5 class=" mb-0 text-gray-800 pl-3">{{ __('Add New Branch') }}
      <a class="btn btn-primary btn-rounded btn-sm" href="{{route('admin.subinstitution.branches',$data->id)}}"><i class="fas fa-arrow-left"></i> {{ __('Back') }}</a>
    </h5>
    <ol class="breadcrumb m-0 py-0">
      <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>
      <li class="breadcrumb-item"><a href="{{ route('admin.subinstitution.index') }}">{{ __('Sub Institutions List') }}</a></li>
      <li class="breadcrumb-item"><a href="{{ route('admin.subinstitution.branches',$data->id)}}">{{ __('Braches List') }}</a></li>
    </ol>
  </div>
</div>
      ...
      <div class="card-body">
        ...
        <form class="geniusform" action="{{route('admin.branch.store')}}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <input type="text" pattern="[^À-ž()/><\][\\;&$@!|]+" class="form-control" id="inp-branch-name" name="branch_name" placeholder="{{ __('Enter Branch Name') }}" value="" autocomplete="off" required>
          </div>
          <!-- </div> -->
          <!-- </div> -->
