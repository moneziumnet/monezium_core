...
<div class="card">
	...
	<h5 class=" mb-0 text-gray-800 pl-3">{{ __('AoA List') }}</h5>
	<ol class="breadcrumb">
		<li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>

		<li class="breadcrumb-item"><a href="{{ route('admin.contract.management', $data->id) }}">{{ __('Contracts') }}</a></li>

        <li class="breadcrumb-item"><a href="{{ route('admin.aoa.index', $id) }}">{{ __('AoA') }}</a></li>
	</ol>
	</div>
</div>
...
<script type="text/javascript">
    ...
    var table = $('#geniustable').DataTable({
           ...
           serverSide: true,
           searching: true,
           ajax: '{{ route('admin.aoa.datatables', $id) }}',
           columns: [
                {
                    "render": function() {
                        return i++;
