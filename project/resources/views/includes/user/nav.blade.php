        ...
        <a class="nav-link" href="{{route('user.shop.index')}}">
         ...
         @if(isEnabledUserModule('Loan') && !(auth()->user()->kyc_status != 1 && in_array('Loan',$kyc_modules)))
          <li class="nav-item dropdown {{ request()->routeIs('user.loans.plan') ? 'active' : '' }}">
            ...
            <li class="nav-item dropdown {{ request()->routeIs('user.merchant.index', 'user.merchant.send.money.create', 'user.merchant.money.request.index', 'user.merchant.other.bank', 'user.merchant.shop.index', 'user.merchant.shop.view_product', 'user.merchant.checkout.index', 'user.merchant.product.edit', 'user.merchant.product.index', 'user.merchant.product.order', 'user.merchant.product.order_by_product', 'user.merchant.checkout.transactionhistory', 'user.merchant.campaign.index', 'user.merchant.campaign.edit', 'user.merchant.campaign.donation_list', 'user.merchant.setting') ? 'active' : '' }}">
              <a class="nav-link dropdown-toggle" href="#navbar-extra" data-bs-toggle="dropdown" data-bs-auto-close="outside" role="button" aria-expanded="false">
                <span class="nav-link-icon d-md-none d-lg-inline-block">
