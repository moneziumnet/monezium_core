...
<div class="card">
    <div class="d-sm-flex align-items-center justify-content-between py-3">
        <h5 class=" mb-0 text-gray-800 pl-3">{{ __('KYC Details') }} <a class="btn btn-primary btn-rounded btn-sm" href="{{route('staff.user.kycinfo',$user->id)}}"><i class="fas fa-arrow-left"></i> {{ __('Back') }}</a></h5>
        <ol class="breadcrumb m-0 py-0">
            ...
            <li class="breadcrumb-item"><a href="{{route('staff.user.kycinfo',$user->id)}}">{{ __('Profile') }}</a></li>
        </ol>
    </div>
</div>
                    ...
                    <div class="footer-area">
                        @if ($kycInformations != NULL)
                            @if ($user->kyc_status == 0)
                                <a href="javascript:;" data-toggle="modal" data-target="#statusModal" data-href="{{ route('staff.user.kyc',['id1' => $user->id, 'id2' => 1]) }}" class="btn btn-primary"><i class="far fa-check-circle"></i> {{__('Approve')}}</a>
                                <a href="javascript:;" data-toggle="modal" data-target="#statusModal" data-href="{{ route('staff.user.kyc',['id1' => $user->id, 'id2' => 2]) }}" class="btn btn-danger ml-3"><i class="fas fa-minus-circle"></i> {{__('Reject')}}</a>
                            @endif
                        @endif
                    </div>
                </div>
