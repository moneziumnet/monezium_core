...
class OwnTransferController extends Controller
{
    ...
    public function transfer(Request $request)
    {
        ...
        send_notification(auth()->id(), amount($request->amount, $fromWallet->currency->type, 3).$fromWallet->currency->code.' Money is exchanged to '.amount($request->amount-($transaction_global_cost +  $transaction_custom_cost)*$rate,$toWallet->currency->type,3).$toWallet->currency->code.'. Please check .', route('admin-user-transactions', auth()->id()));
