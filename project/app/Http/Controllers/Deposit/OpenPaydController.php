...
class OpenPaydController extends Controller
{
    ...
    if ($linked_accountid == null) {
        ...
        if ($iban == null || $bic_swift == null || $iban == '' || $bic_swift == '' ) {
            return redirect()->back()->with(array('warning' => 'Sorry, You can\'t create New Bank Account succesfully because openpayd\'s Iban and Swift code are not generated by OPENPAY API. Please try again.'));
        }
    ...
    public function store_uk(Request $request){
        $gs = Generalsetting::first();
        $client = New Client();
        $user = User::findOrFail($request->user);
        $bankgateway = BankGateway::where('subbank_id', $request->subbank)->first();
        ...
        $user->holder_id_uk = $linked_accountid;
        $user->update();
    }
    else {
        $linked_accountid = $user->holder_id_uk;
    }
