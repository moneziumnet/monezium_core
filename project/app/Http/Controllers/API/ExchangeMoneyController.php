...
class ExchangeMoneyController extends Controller
{
    ...
    public function exchangeForm()
    {
        try {
            ...
            $user = auth()->user();
            return response()->json(['status' => '200', 'error_code' => '0', 'message' => 'success', 'data' => compact('wallets', 'currencies', 'recentExchanges', 'crypto_currencies', 'user', 'rate')]);
        }
        catch (\Throwable $th) {
        ...
        try {
            ...
            if (!$toWallet) {
                if ($currency->type == 2) {
                    if ($currency->code == 'BTC') {
                    ...
                    } else if ($currency->code == 'USDT(TRON)' && $currency->curr_name == 'Tether USD TRC20') {
                        ...
                        if (!$tron_wallet) {
                            return response()->json(['status' => '401', 'error_code' => '0', 'message' => 'Now, You do not have TRON Crypto Wallet. You have to create TRON Crypto wallet firstly for this exchange action .']);
                        }
                        ...
                        if (!$eth_wallet) {
                            return response()->json(['status' => '401', 'error_code' => '0', 'message' => 'Now, You do not have Eth Crypto Wallet. You have to create Eth Crypto wallet firstly for this exchange action .']);
                        }
            ...
            if ($toWallet->currency->type == 2) {
                if ($finalAmount > Crypto_Balance(0, $toWallet->currency->id)) {
                    return response()->json(['status' => '401', 'error_code' => '0', 'message' =>  'Insufficient balance to this system ' . $toWallet->currency->code . ' wallet, pleasae contact to admin.']);
                }
            }
            ...
            if ($fromWallet->currency->code == 'ETH' && $toWallet->currency->type == 1) {
                ...
                if (!$tosystemwallet1) {
                    return response()->json(['status' => '401', 'error_code' => '0', 'message' =>  $toWallet->currency->code . ' System Account does not exist. you can not exchange now. Please contact to support team. ']);
                }
    ...
    public function calcharge(Request $request)
    {
        try {
            $user = auth()->user();
            $transaction_global_cost = 0;
            if ($request->fromtype == 1 && $request->totype == 1) {
                $transaction_global_fee = check_global_transaction_fee($request->amount, $user, 'exchange');
            } else if ($request->fromtype == 2 && $request->totype == 1) {
                $transaction_global_fee = check_global_transaction_fee($request->amount, $user, 'exchange_c_f');
            } else if ($request->fromtype == 1 && $request->totype == 2) {
                $transaction_global_fee = check_global_transaction_fee($request->amount, $user, 'exchange_f_c');
            } else if ($request->fromtype == 2 && $request->totype == 2) {
                $transaction_global_fee = check_global_transaction_fee($request->amount, $user, 'exchange_c_c');
            }
            if ($transaction_global_fee) {
            ...
            if ($user->referral_id != 0) {
                ...
                if ($request->fromtype == 1 && $request->totype == 1) {
                    ...
                    $transaction_custom_fee = check_custom_transaction_fee($request->amount, $user, 'exchange_c_f');
                } else if ($request->fromtype == 1 && $request->totype == 2) {
                    $transaction_custom_fee = check_custom_transaction_fee($request->amount, $user, 'exchange_f_c');
                } else if ($request->fromtype == 2 && $request->totype == 2) {
                    $transaction_custom_fee = check_custom_transaction_fee($request->amount, $user, 'exchange_c_c');
                }
                if ($transaction_custom_fee) {
            ...
            return response()->json(['status' => '401', 'error_code' => '0', 'message' => 'success', 'data' => ['finalcharge' => $finalCharge]]);
        } catch (\Throwable $th) {
            return response()->json(['status' => '401', 'error_code' => '0', 'message' => $th->getMessage()]);
        }
    ...
    public function exchangeHistory()
    {
        try {
            ...
            return response()->json(['status' => '200', 'error_code' => '0', 'message' => 'success', 'data' => compact('exchanges', 'search')]);
        } catch (\Throwable $th) {
            return response()->json(['status' => '401', 'error_code' => '0', 'message' => $th->getMessage()]);
        }
    }
