...
class UserICOController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            mailSend('ico_create',[ 'date_time'=>$data->created_at, 'code' => $currency->code, 'symbol' => $currency->symbol, 'total_supply' => $data->total_supply, 'amount' => $data->price, 'curr' => $def_currency->code], auth()->user());

            send_notification(auth()->id(), 'New ICO token has been created by '.(auth()->user()->company_name ?? auth()->user()->name)."\nICO Token Name is ".$data->name."\nTotal Supply is".$data->total_supply."\nPrice is ".$def_currency->symbol.$data->price."\n Please check.", route('admin.ico.index'));
            send_staff_telegram('New ICO token has been created by '.(auth()->user()->company_name ?? auth()->user()->name)."\nICO Token Name is ".$data->name."\nTotal Supply is".$data->total_supply."\nPrice is ".$def_currency->symbol.$data->price."\n Please check.\n".route('admin.ico.index'), 'ICO');
